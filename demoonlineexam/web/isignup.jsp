...
out.println("<h3>Loginname has already been taken.Try with a different Login name.</h3></td></tr>");
...
s.executeUpdate("insert into org_details (LoginName,Email,Password,InstituteName) values('"+loginname+"','"+email+"','"+password+"','"+iname+"')");
String s1=loginname+"edetails";
String s2=loginname+"qtable";
String s3=loginname+"slist";
s.executeUpdate("create table "+s1+"(ID number,Ename text,Tlimit text,Passmarks number,NoQues number,Pmarks number,Nmarks number)");
s.executeUpdate("create table "+s2+"(QID Counter,Question text,A text,B text,C text,D text,Answer text)");
s.executeUpdate("create table "+s3+"(SID number,Sname text,Egiven number,marks number,result text)");
...
out.println("<tr><td>&nbsp;</td></tr><tr><td>&nbsp;</td></tr><tr><td>&nbsp;</td></tr>");
out.println("<tr><td></td><td align=left><a href='afterisignup.jsp?name="+loginname+"&lid="+id+"'><img src=ContinueButton.jpg width=130height=40></a>");
